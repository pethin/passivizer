...
2. **Dynamic Magnetic Feel:** Model register-weighted magnetic string pull damping and attack pitch sag ($w_{\text{reg}}$ in `_lenz_velocity_drag_core`), displacement-modulated touch highpass tilt ($\tau_{\text{touch}}$), eddy de-Qing, $2f_0$ orbit bloom, core curvature wobble, soft-knee slew limiting, conformal clearance divergence, and $-108\text{ dBFS}$ Johnson noise dither.
