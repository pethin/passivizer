...
1. **Two-Stage Signal Flow Decoupling:**
   ...
   - **Stage 2 (Backend NAM):** Captures target pickup loaded RLC circuit and non-linear magnetic feel from the Canonical Intermediate baseline. Loaded into Block 2 (NAM Preamp, A2-Lite architecture).
       - **A2-Lite Studio Reference Standard:** Governed by $\text{ESR} \le 0.0005$ ($\approx -33\text{ dB}$ residual error) with a $500$ max epoch safety ceiling and batch size $32$. In time-domain mean-squared error ($\text{ESR} = \sum (y - \hat{y})^2 / \sum y^2$), low-frequency fundamentals carry $80\text{--}90\%$ of total signal power. By default, training isolates the 8-channel A2-Lite submodel (`channels_8`), cutting parameters from $22.8\text{ K} \to 12.1\text{ K}$, accelerating epoch throughput by $2.0\times$, and ensuring reported validation ESR accurately measures the 8-channel model without artificial inflation from the 3-channel nano submodel. Full slimmable containers containing both submodels are selectable via `--a2-full`. Deep convergence ($\le 0.0005$) ensures the optimizer resolves high-frequency pickup RLC resonant peaks ($3\text{--}5\text{ kHz}$), pick attack transients, and dual-pickup comb filtering without premature cutoff or perceptual high-frequency haziness.
